...
class SlackConnector(BaseConnector):
    ...
    platform_name = "slack"
    supports_push = False  # Read-only ingestion via Events API
    ...
    def supports_push(self) -> bool:
        """Slack uses push via Events API (read-only)."""
        return False
    ...
    async def put_item(self, item: ConnectorItem) -> str:
        """Push item to Slack (not supported for read-only ingestion)."""
        raise NotImplementedError("Slack connector is read-only")

    async def delete_item(self, item_id: str) -> bool:
        """Delete item from Slack (not supported)."""
        raise NotImplementedError("Slack connector is read-only")
    ...
    def transform_from_claim(self, claim: dict) -> ConnectorItem:
        """Convert SAW Claim to Slack item (not supported)."""
        raise NotImplementedError("Slack connector is read-only")
