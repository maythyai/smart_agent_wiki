...
class WorkflowExecutor:
    ...
    async def resume(
        ...
        """Resume an interrupted/failed workflow from its persisted step index.

        Per ADR-006: index-based resume. Reads the ``workflow_executions`` row,
        validates the state transition (INTERRUPTED/FAILED/TIMEOUT → RUNNING),
        re-parses the definition, and re-executes from ``steps_completed``.
        Context is not persisted, so already-completed steps' outputs are not
        reconstructed — the next step starts with a fresh context (any
        dependent output is regenerated by re-running that step).

        Args:
            workflow_id: The persisted workflow_id to resume.
            workflow_path: Path to the workflow YAML (re-parsed; the table
                stores only ``definition_name``, not the full definition).
            inputs: Input context for the resumed run.

        Returns:
            WorkflowResult with execution status and outputs.

        Raises:
            RuntimeError: If no conn was wired, the row is missing, or the
                state transition is illegal (e.g. resuming a COMPLETED run).
        """
        if self._conn is None:
            raise RuntimeError(
                "resume requires a durable connection (conn=None); "
                "re-run with a wired DB connection"
            )
        row = self._conn.execute(
            "SELECT status, steps_completed, steps_total, definition_name "
            "FROM workflow_executions WHERE workflow_id = ?",
            (workflow_id,),
        ).fetchone()
        if row is None:
            raise RuntimeError(f"No workflow_executions row for id={workflow_id}")
        prev_status, steps_completed, steps_total, _name = row
        if not validate_workflow_transition(prev_status, WorkflowStatus.RUNNING):
            raise RuntimeError(
                f"Cannot resume workflow {workflow_id}: "
                f"illegal transition {prev_status} -> running "
                f"(only interrupted/failed/timeout/pending may resume)"
            )
        workflow_def = self._parser.parse(workflow_path)
        start = int(steps_completed or 0)
        # Clamp to valid range (def may have changed; never go negative/over).
        start = max(0, min(start, len(workflow_def.steps)))
    ...
    async def execute_definition(
        ...
        *,
        ...
        Args:
            ...
            inputs: Input context for workflow
            workflow_id: Optional existing workflow_id (resume path).
                When None a fresh uuid is generated.
            start_step: Index of the first step to run (resume path). Steps
                before this index are assumed already completed (HI-9 resume).
                Defaults to 0 (run from the start).
        ...
        self._persist_workflow(
            workflow_id, workflow.name, "running", steps_completed, len(workflow.steps), []
        )
