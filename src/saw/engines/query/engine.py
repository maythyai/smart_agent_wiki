...
# S4: semantic search (cosine + ANN) extracted to a mixin to slim the
# god-file; the mixin accesses engine state via self (effective_workspace_id,
# _keyword_search, _claims_repo, _wiki_repo, _conn). Imported lazily inside
# the mixin to avoid a circular import (semantic.py imports QueryResult).
from saw.engines.query.semantic import SemanticSearchMixin  # noqa: E402
...
class QueryEngine(SemanticSearchMixin):
