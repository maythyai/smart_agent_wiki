...
class FlowTracer:
    ...
    def _trace_single_flow(self, entry_uid: str, max_depth: int) -> Optional[ExecutionFlow]:
        ...
        flow.has_tests = self._check_test_coverage(entry_uid)
    ...
    def _score_criticality(self, flow: ExecutionFlow) -> float:
        ...
        # 测试覆盖 (有测试降 0.1)
        if self._check_test_coverage(flow.entry_point):
            score -= 0.1
    ...
    def _check_test_coverage(self, uid: str) -> bool:
        """检查节点是否有测试覆盖"""
        tests = self.store.get_incoming_edges(uid, [EdgeType.TESTED_BY.value])
        return len(tests) > 0
