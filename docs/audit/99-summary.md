...
| Batch 40 | F-INGEST-15 + F-INGEST-16 + F-CLI-06 | ✅ 3 fix | 离线截断 >10 段落现 log warning（原静默）；types.py datetime.utcnow()→datetime.now(timezone.utc)（消除弃用）；error_handler fallback 由"Run saw --help"改为含异常类型+可操作建议 |
...
> 验证：Batch 7-40 全程 0 回归，后端 1569 / 前端 51 / tsc 干净 / MCP 61。
> P3 验证（Batch 41-42）：页面暗色 gap 已闭环、Pages/Search 空状态已有引导、CLI 无原始 UUID 显示、slug 中英混用为项目设计。无需修复。
> **最终统计：42 批 / 42 commit / ~77 条 finding / 0 回归 / 全部 P0+P1+P2+P3 处置完毕。**
